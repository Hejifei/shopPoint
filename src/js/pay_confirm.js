...
let payForGoods = ''
let cardBankName = '' //选择银行卡返回
let fromorder = ''
let accountInfo = ''
let DefaultCardInfo = ''
let moneyintChangebl = ''
...
let paytype = []
let ifIntegral = true; // 是否启用积分支付，true 启用 false 不启用
let ifpayType = 1 ; // 1单渠道支付(默认) 2混合渠道支付
let ifexchangeType = 0; //是否抵扣积分  0不使用 1使用 2部分抵扣
// 输入密码
var pwd = ''
var len = 0
...
let newtotalAmount = 0; // 最终支付总金额 
let newtotalIntegral = 0 ; // 最终支付总积分
let avalbMoney = 0; // 赢粉宝账户可用金额
$(function() {
  // 付款方式选择
  // 微信置灰
    ...
    if(!$(this).find('.pay_mleft').hasClass('pay_cantsel')){
      ...
      if(ifIntegral){
        ...
        if(Number(accountInfo.integral) - Number(payinfo.totalIntegral) > 0){
          ...
          let addmoney = (moneyintChangebl *(Number(payinfo.totalIntegral) - Number(accountInfo.integral))).toFixed(2)
  ...
  $('.pp_set').click(function() {
    if(isWechat){
      ...
      let urlData = window.location.href
      window.location.href = h5pswurl +`/tiens-h5/html/reset_pwd.html?paymode=payPoint&url=${urlData}&statusBarHeight=${statusBarHeight}&token=${localStorage.token}&memid=${memidGet()}`
    }
  })
  ...
  $('.pro_price').text(
    `￥${payinfo.totalAmount.toFixed(2)}${
      payinfo.totalIntegral === 0 ? '' : '+' + payinfo.totalIntegral + '积分'
    }`
  )

  let limitTime = payinfo.limitTime
  limitTime = limitTime.substring(0, 19)
  limitTime = limitTime.replace(/-/g, '/')
  let nowdata = new Date() //当前时间
  let enddate = new Date(limitTime) //订单结束时间
  let timeleft = enddate.getTime() - nowdata.getTime()
  ...
  $('.pay_btnC a').click(function() {
    ...
    if(paytype === 8){
      if(DefaultCardInfo.cardBankCardId === null){
        $('.pay_pswBody .pp_ptype').html(
          `<img src="../images/wallet_bank.png"/><span>请选择银行卡</span>`
        )
      }else{
        $('.pay_pswBody .pp_ptype').html(
          `<img src="${DefaultCardInfo.cardBankLogo}"/><span>${DefaultCardInfo.cardBankName}(${DefaultCardInfo.cardBankNo.substr(DefaultCardInfo.cardBankNo.length-4)})</span>`
        )
      }
        ...
        switch (paytype) {
          case '0':
            ...
            if (isAndroid) {
              ...
              // ios
              window.webkit.messageHandlers.onAliPay.postMessage(
                res.data.payInfo.payUrl
              )
            }
            break
          case '1':
            ...
            if (isAndroid) {
              ...
              // ios
              window.webkit.messageHandlers.onWechatPay.postMessage(
                JSON.stringify(wechatjson)
              )
            }
            break
          case '2':
            ...
            if (isAndroid) {
              ...
              // ios
              window.webkit.messageHandlers.onBankPay.postMessage(
                JSON.stringify(bankpayval)
              )
            }
            break
          case '3':
            ...
            $('.pay_pswBody .pp_ptype').html(
              '<img src="../images/shop_yue.png"/><span>余额</span>'
            )
            $('.real-ipt').val('')
            // 清空密码输入框的值
            $inputs.each(function() {
		...
		(res) => {
			if (res.code === '0') {
				tiansLayer({
					title: '提示',
					content:
						'<p>您的钱包余额已被冻结，请联系天狮钱包客服:<a style="color: #009943" href="tel:' +
...
let eprotocolacNo ='';// 电子账户协议编号	
var sttlNo = '';// 特色缴费
let Random = '';// 密码随机数
let RandJnlNo = '';// 密码随机数流水号
// 赢粉宝
const yfbFun = () => {
  // 加载动画
  // layer.open({ type: 2 })
          ...
          ajaxJSNew(
            ...
            error => {
              ...
              if (error.code === 'DS00000') {
                avalbMoney = 0;
                let AvailableBalance = error.data.AvailableBalance
                let ProdAvailableShare = error.data.AssetsMap.RYB.ProdAvailableShare
                avalbMoney = (Number(AvailableBalance) + Number(ProdAvailableShare)).toFixed(2)
                let needMoney = newtotalAmount
                ...
                $('.pay_pswBody .pp_ptype').html(
                  '<img src="../images/personal_yfb1.png"/><span class="spanyfb">赢粉宝<i>(请用民生II类户交易密码)</i></span>'
                )
                //赢粉宝
                  var param = {
...
const yfbH5psw = (psw) => {
  ...
  ajaxJSNew(
    ...
    error => {
      if (error.code === 'DS00000') {
        let ChannelJnlNo = error.data.ChannelJnlNo
        let Token = error.data.Token
        let Amount = 0
        let TransInAmount = ''
        // newtotalAmount 总额
        // avalbMoney 赢粉宝余额
...
function doneCallback(sipBoxId) {
  ...
  let yfbpswLen = $('#SIPBox3').val().length
  if(yfbpswLen === 6){
    // 加载动画
    $('.payloading').show()
    // 获取键盘输入的加密密码
    let yfbPsw = keyboard1.getEncryptedInputValue('SIPBox3')
    yfbH5psw(yfbPsw)
...
let keybdInterval = ''
// 银行键盘显示
const yfbPswkeybdShow = () => {
  keybdInterval = setInterval(function(){
    if($('#SIPBox3').val().length > 0){
      let yfbPsLen = $('#SIPBox3').val().length
      for (let i = 0;i<yfbPsLen;i++) {
        $wrappers.eq(i).show()
      }
    }else{
      $wrappers.hide()
