...
$(function() {
  ...
  let addressId = getUrl(location.href).addressId
  if (addressId !== undefined && localStorage.adsEdit) {
    let adsEdit = JSON.parse(localStorage.adsEdit)
    ...
    $('#defaultStatus').prop(
      'checked',
      adsEdit.defaultStatus === 1 ? true : false
    )
  ...
  $('.submit_btn').click(function() {
    ...
    if (
      $('#detailAddress').val().length < 5 ||
      $('#detailAddress').val().length > 50
    ) {
      tiansLayer({
        title:'提示',
