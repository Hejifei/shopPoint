...
$(function () {
  ...
  if (!isWechat) {
    ...
    $('.find_search_list_item').css('height', '-webkit-calc(100vh - ' + ((92 + statusBarHeight) / 75).toFixed(4) + 'rem)');
    $('.prolistOutC').css('height', '-webkit-calc(100vh - ' + ((172 + statusBarHeight) / 75).toFixed(4) + 'rem)');
    $('.noresult').css('height', '-webkit-calc(100vh - ' + ((92 + statusBarHeight) / 75).toFixed(4) + 'rem)');
    $('.navigation').css({
  ...
  if (layoutId === undefined) {
    // 搜索
    if (sessionStorage.searchName !== undefined && sessionStorage.searchName !== '') {
  ...
  $('.menuUl li').click(function () {
    ...
    switch ($(this).index()) {
      ...
      case 3:
        ...
        for (var _i in arr) {
          if ($(this).hasClass('menusel_re')) {
            if (Number(querydata.searchType) === arr[_i].searchType) {
    ...
    tiansLayer({
      ...
      yes: function yes() {
        ...
        ajaxJS(param, function (res) {
          getSearchHistory();
          layer.open({
            title: ['提示', 'background-color: #fff; color:#030303;border-bottom:1px solid #009943;'],
            content: res.msg,
            btn: ['我知道了'],
  ...
  $('.navright').click(function () {
    ...
    var type = getUrl(location.href).tType;
    if (type === '1' || type === '3') {
      // 未登录的跳登陆页
      if (localStorage.token === undefined || localStorage.scMemId === undefined) {
        if (isWechat) {
  ...
  ajaxJS(param, function (res) {
    ...
    if (res.data.length !== 0) {
      ...
      for (var i in res.data) {
        arrlist += '<li seahis_id=\'' + res.data[i].id + '\'>' + res.data[i].searchKey + '</li>';
      }
...
$(function () {
  var arr = [{ name: '默认排序', searchType: 0 }, { name: '价格从高到低', searchType: 1 }, { name: '价格从低到高', searchType: 2 }, { name: '新品优先', searchType: 4 }];
  $('.search-top').find('div').click(function (e) {
    e.stopPropagation();
    $('.search-top').find('div').each(function (i, item) {
