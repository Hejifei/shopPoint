...
$(function () {
  ...
  $('.classify_two').delegate('.menuSec .menuSec_son li', 'click', function () {
    $(this).siblings('.menuSec_sonactive').removeClass('menuSec_sonactive');
    $(this).addClass('menuSec_sonactive');
    queryData.categoryId = $(this).attr('typeId');
    queryData.page = 1;
    $('.menutitle').html($(this).text() + '<img  src="../images/general_xiala.png"/>');
    $('.menuSec_son').hide();
    getGoodsByTypeId(queryData);
  });
  ...
  $('.classify_one ul').delegate('li', 'click', function () {
    ...
    var parentid = Number($(this).attr('typeId'));
    var menuseclist = Goodslist.filter(function (val) {
      if (val.id === parentid) {
        return val;
      }
    });
    ...
    if (menuseclist[0].children.length > 0) {
      for (var j in menuseclist[0].children) {
        menulistson += '<li class=\'' + (j === '0' ? 'menuSec_sonactive' : '') + '\' typeId=\'' + menuseclist[0].children[j].id + '\' >' + menuseclist[0].children[j].typeName + '</li>';
      }
    ...
    if (menuseclist[0].children.length > 0) {
      $('.menutitle').html(menuseclist[0].children[0].typeName + '<img  src="../images/general_xiala.png"/>');
    } else {
      $('.menutitle').html('');
    }
  ...
  $('.adv_img').click(function () {
    ...
    } else if ($(this).attr('atype') === '1' && $(this).attr('externalUrl') !== '') {
      // 外部链接
      if ($(this).attr('externalUrl').indexOf('/html/search.html?layoutId=') !== -1) {
        // 内部链接
        var layoutId = getUrl($(this).attr('externalUrl')).layoutId;
        var layoutName = getUrl($(this).attr('externalUrl')).layoutName;
  ...
  ajaxJS(param, function (res) {
    ...
    if (res.data.length > 0) {
      ...
      for (var i in res.data) {
        arr += '\n                <li  class="opacity ' + (i === '0' ? 'active' : '') + '" typeId=\'' + res.data[i].id + '\'>\n                    <div class="item_one">\n                        <div class="bar"></div>\n                        <span>' + res.data[i].typeName + '</span>\n                    </div>\n                </li>\n                ';
      }
      var menulistson = '';
      if (res.data[0].children.length > 0) {
        for (var j in res.data[0].children) {
          menulistson += '<li class="' + (j === '0' ? 'menuSec_sonactive' : '') + '" typeId=\'' + res.data[0].children[j].id + '\' >' + res.data[0].children[j].typeName + '</li>';
        }
      }
      // 左侧菜单
      $('.classify_one ul').html('').html(arr);
      // 左侧第一菜单的二级菜单选项
      $('.menuSec_son').html(menulistson);
      // 没有图片时隐藏顶部
      if (res.data[0].pic === '') {
      ...
      $('.menutitle').html(res.data[0].children[0].typeName + '<img src="../images/general_xiala.png"/>');
  ...
  ajaxJS(param, function (res) {
    ...
    if (res.data !== null && res.data.list !== null) {
      for (var i in res.data.list) {
        arr += '\n                <li class="opacity">\n                    <a href="./shop_detail.html?id=' + res.data.list[i].id + '">\n                        <div class="imgs">\n                            <img  class="lazy" data-original="' + res.data.list[i].goodsImageUrl + '" alt="">\n                        </div>\n                        <div class="detail">\n                            <div class="title">' + res.data.list[i].goodsName + '</div>\n                            <div class="introduce">' + res.data.list[i].introduce + '</div>\n                            <div class="oldPrice">\xA5 ' + res.data.list[i].originalPrice + '</div>\n                            <div class="more">\n                                <div class="price">\xA5 ' + res.data.list[i].showPrice + '</div>\n                                <div class="discount">' + (res.data.list[i].integralAmount === 0 || res.data.list[i].integralAmount === null ? '' : '+积分:' + res.data.list[i].integralAmount) + '</div>\n                                <div class="add" goodsId=\'' + res.data.list[i].id + '\'><img  src="../images/main_add.png" alt=""></div>\n                            </div>\n                        </div>\n                    </a>\n                </li>\n                ';
      }
    } else {
