...
var shopId = getUrl(location.href).shopId;
...
$(function () {
  ...
  if (!isWechat) {
    ...
    $('.content').css('height', '-webkit-calc(100vh - ' + ((310 + statusBarHeight) / 75).toFixed(4) + 'rem)');
  }
  ...
  $('.menuUl li').click(function () {
    ...
    switch ($(this).index()) {
      case 0:
        var arr = [{ name: '默认排序', searchType: 0 }, { name: '价格从高到低', searchType: 1 }, { name: '价格从低到高', searchType: 2 }, { name: '新品优先', searchType: 4 }];
        var menuSon = '';
        for (var i in arr) {
          if ($(this).hasClass('menusel_re')) {
            if (Number(querydata.searchType) === Number(arr[i].searchType)) {
              menuSon += "<li searchType='" + arr[i].searchType + "' class='menusonSel'>" + arr[i].name + '</li>';
            } else {
              menuSon += "<li searchType='" + arr[i].searchType + "'>" + arr[i].name + '</li>';
            }
          } else {
            menuSon += "<li searchType='" + arr[i].searchType + "'>" + arr[i].name + '</li>';
          }
      ...
      case 1:
        ...
        querydata.searchName = $('#searchName').val(), querydata.searchType = $(this).attr('searchType');
        querydata.page = 1;
      ...
      case 2:
        ...
        break;
      case 3:
        var arr = [{ name: '价格从高到低', searchType: 1 }, { name: '价格从低到高', searchType: 2 }];
        var menuSon = '';
        for (var _i in arr) {
          if ($(this).hasClass('menusel_re')) {
            if (Number(querydata.searchType) === Number(arr[_i].searchType)) {
              menuSon += "<li searchType='" + arr[_i].searchType + "' class='menusonSel'>" + arr[_i].name + '</li>';
            } else {
              menuSon += "<li searchType='" + arr[_i].searchType + "'>" + arr[_i].name + '</li>';
            }
          } else {
            menuSon += "<li searchType='" + arr[_i].searchType + "'>" + arr[_i].name + '</li>';
          }
  ...
  $('.home_content_content').delegate('.add', 'click', function (ev) {
    ...
    // 加入购物车动画
    var flyElm = $(this).parents('li').find('.img img').clone().css('opacity', '0.8');
    flyElm.css({
      'z-index': 9000,
      display: 'block',
      position: 'absolute',
      top: $(this).parents('li').find('.img img').offset().top + 'px',
      left: $(this).parents('li').find('.img img').offset().left + 'px',
      width: $(this).parents('li').find('.img img').width() + 'px',
      height: $(this).parents('li').find('.img img').height() + 'px'
    });
    $('body').append(flyElm);
    flyElm.animate({
...
// 排序选项及文字恢复
var menulistrenew = function menulistrenew() {
  // querydata.page = 1;
...
var shopInfoGet = function shopInfoGet() {
  var param = {
  ...
  ajaxJS(param, function (res) {
    $('.shop_detail .title').html('<span class="opacity">' + res.data.shopName + '</span><a href=\'./shopInfo.html?shopId=' + shopId + '\'>\u5546\u5BB6\u4FE1\u606F<img src="../images/general_next.png" /></a>');
    var starlist = '';
    if (res.data.starLevel > 0) {
      for (var i = 0; i < res.data.starLevel; i++) {
        starlist += '<img class=\'opacity\' src="../images/shop_level.png" />';
      }
    }
    $('.home_top').css({
      background: 'url(' + res.data.backUrlImage + ') no-repeat center center',
...
// 店铺主页商品
var shopGoodsGet = function shopGoodsGet(querydata) {
  var param = {
  ...
  ajaxJS(param, function (res) {
    ...
    if (res.data.list !== null) {
      ...
      for (var i in res.data.list) {
        arrlist += '\n                <li class="opacity">\n                    <a href=\'./shop_detail.html?id=' + res.data.list[i].goodsId + '\'>\n                        <div class="img">\n                          <div><img class="lazy" data-original="' + res.data.list[i].imageUrl + '" alt=""></div>\n                        </div>\n                        <div class="detail">\n                            <div class="title">' + res.data.list[i].goodsName + '</div>\n                            <div class="more">\n                                <div class="price">\xA5 ' + res.data.list[i].showPrice + '</div>\n                                ' + (res.data.list[i].integralPrice === null || res.data.list[i].integralPrice === 0 ? '' : '<div class="discount">+积分:' + parseInt(res.data.list[i].integralPrice) + '</div>') + '\n                                <div class="add" goodsid=\'' + res.data.list[i].goodsId + '\'><img src="../images/main_add.png" /></div>\n                            </div>\n                        </div>\n                    </a>\n                </li>\n                ';
      }
      arrlist += '</ul>';
    } else {
      totalpages = 0;
...
var fromShareFun = function fromShareFun() {
  var shareUrl = window.location.href + '&download=1';
