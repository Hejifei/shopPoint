...
$(function () {
  ...
  $('.prolist').delegate('.assessline a', 'click', function () {
    $(this).parent('.assessline').find('.assessactive').removeClass('assessactive');
  ...
  $('.assessSubC a').click(function () {
    ...
    $('.prolist .prolist_son').each(function () {
      commentlist.push({
        subOrderNo: $(this).find('textarea').attr('subOrderNo'),
        level: $(this).find('.assessactive').attr('level'),
        evaluate: $(this).find('textarea').val().trim(),
        productName: $(this).find('textarea').attr('productName'),
        orderNo: orderNo,
    ...
    ajaxJS(param, function (res) {
      if (res.code === '0') {
        ...
        for (var i in commentlist) {
          productevaluateTracker('', '', '', '', commentlist[i].goodsId, commentlist[i].goodsName, commentlist[i].level === '1' ? 1 : 5);
        }
        tiansLayer({
          title: '提示',
